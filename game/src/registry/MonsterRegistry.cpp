...
void registerAllMonsters() {
    ...
    {   EnemyDef def("Gust Wraith", 45, 8, 4, Track::EcoTech);
        ...
        addCardByName(def, cards, "연막");  // Self evade — 적 자신에게 적용
        addCardByName(def, cards, "윈드시어");
        reg.registerMonster(def); }

    {   EnemyDef def("Turbine Beast", 90, 13, 8, Track::EcoTech);
        addCardByName(def, cards, "터빈 킥");
        addCardByName(def, cards, "순풍");    // Party ATK 버프 — 적 파티에 적용됨 (sourceIsPlayer=false)
        addCardByName(def, cards, "강타");
        reg.registerMonster(def); }
    ...
    {   EnemyDef def("Neural Phantom", 95, 11, 6, Track::AI);
        ...
        addCardByName(def, cards, "긴급 패치");      // 자가 HP 회복
        reg.registerMonster(def); }
    ...
    {   EnemyDef def("Overload Titan", 130, 14, 10, Track::Grid);
        ...
        addCardByName(def, cards, "축전기");   // 자가 ATK 버프 + 방어막
    ...
    {   EnemyDef def("NEXUS Core", 300, 20, 15, Track::AI, true);
        ...
        addCardByName(def, cards, "암호화");  // 자가 block + DEF 버프
    ...
    {   EnemyDef def("Storm Colossus", 260, 18, 14, Track::EcoTech, true);
        addCardByName(def, cards, "윈드시어");
        addCardByName(def, cards, "폭풍의 눈"); // 자가 evade + heal
        addCardByName(def, cards, "토네이도");
        addCardByName(def, cards, "대지의 축복"); // Party 힐 — 적 파티에 적용됨 (sourceIsPlayer=false)
        reg.registerMonster(def); }
}
